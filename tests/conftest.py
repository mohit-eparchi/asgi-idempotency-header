...
method_configs = {
    ...
    "with put": {
        "setting": ["POST", "PATCH", "PUT"],
        "applicable_methods": ["post", "patch", "put"],
    },
}
...
@pytest.fixture(
    scope="session", ids=method_configs.keys(), params=method_configs.values()
)
...
def app_with_middleware(method_config):
    ...
    async def create_normal_response():
        return Response(
            content=json.dumps(dummy_response), media_type="application/json"
        )
    ...
    async def create_bad_response():
        return Response(
            content=json.dumps(dummy_response), media_type="application/xml"
        )
...
async def client(app_with_middleware) -> AsyncGenerator[AsyncClient]:
...
def applicable_method(method_config, client, request):
    if request.param in method_config["applicable_methods"]:
        ...
        raise pytest.skip(
            request.param + " is not an applicable method in this configuration."
        )
...
def inapplicable_method(method_config, client, request):
    if request.param in method_config["applicable_methods"]:
        raise pytest.skip(
            request.param + " is an applicable method in this configuration."
        )
    else:
        return client.__getattribute__(request.param)
